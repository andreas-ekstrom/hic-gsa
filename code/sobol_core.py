...
def sensitivity_analysis_plot_multi(
    ...
    Requires: `sensitivity_analysis(problem, X_values, Y_values)` and `pre` colors.
    """
    ...
    results = []
    for k in range(Ny):
        Si_mean, Si_ci, St_mean, St_ci = sensitivity_analysis(problem, X_values, Y_values_list[k])
        results.append({
